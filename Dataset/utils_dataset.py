...
# def _record_net_data_stats(y_train, net_dataidx_map,logger):
#     net_cls_counts = {}
...
#
#     data_list=[]
#     for net_id, data in net_cls_counts.items(): # 遍历所有的参与者
#         n_total=0
#         for class_id, n_data in data.items(): # 遍历参与者的所有类别
#             n_total += n_data
#         data_list.append(n_total)
#     # print('mean:', np.mean(data_list))
#     # print('std:', np.std(data_list))
#     logger.info('Data statistics: %s' % str(net_cls_counts))
#     _visualize_data_distribution(net_cls_counts)
#     return net_cls_counts
#
# def _visualize_data_distribution(net_cls_counts):
#     sns.set(font_scale=1.5)
#     df = pd.DataFrame(net_cls_counts, dtype=int).fillna(0).astype('int')
#     plt.figure()
#     sns.heatmap(data=df, annot=False, fmt="d", cmap="Reds")
#     heatmap_path = Project_Path+ 'Dataset/'
#     plt.savefig(heatmap_path+'Heatmap.png')
...
if __name__ =='__main__':
    ...
    public_data_idxs_list = generate_public_data_idxs(dataset='cifar_100', datadir=Dataset_Dir + 'cifar_100', size=5000, epoch=3)

    # public_data_idxs = generate_public_data_idxs(dataset='cifar_100', datadir=Dataset_Dir + 'cifar_100', size=5000, epoch=None)
    # train_dl, test_dl, train_ds, test_ds = get_dataloader(dataset='cifar_100', datadir=Dataset_Dir + 'cifar_100', train_bs=256, test_bs=512, dataidxs=public_data_idxs_list[0])
    #
    # public_data_idxs = generate_public_data_idxs(dataset='tiny_imagenet', datadir=Dataset_Dir + 'tiny_imagenet', size=5000, epoch=None)
    # train_dl, test_dl, train_ds, test_ds = get_dataloader(dataset='tiny_imagenet', datadir=Dataset_Dir + 'tiny_imagenet', train_bs=256, test_bs=512, dataidxs=public_data_idxs_list[0])

    public_data_idxs = generate_public_data_idxs(dataset='FashionMNIST', datadir=Dataset_Dir + 'FashionMNIST', size=5000, epoch=None)
    train_dl, test_dl, train_ds, test_ds = get_dataloader(dataset='FashionMNIST', datadir=Dataset_Dir + 'FashionMNIST', train_bs=256, test_bs=512, dataidxs=public_data_idxs_list[0])
