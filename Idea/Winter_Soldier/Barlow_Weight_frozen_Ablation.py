...
from Idea.utils_idea import update_model_via_private_data,evaluate_network,mkdirs
...
Ablation_Name='Barlow_Weight_l2_frozen_Ablation' # 使用了_l2
    ...
    for epoch_index in range(CommunicationEpoch):
        ...
        for _ in range(Public_Training_Epoch):
            for batch_idx, (images, _) in enumerate(public_train_dl):
                ...
                linear_output_frozen_list = []
                linear_output_progressive_list = [] # Save itself progressive model's linear output
                linear_ouput_softmax_list = []
                linear_output_frozen_softmax_list = []
                linear_output_progressive_softmax_list = []
                col_loss_batch_list = []
                '''
                ...
                for participant_index in range(N_Participants):
                    ...
                    linear_ouput_softmax_list.append(linear_output_softmax.clone().detach())
                '''
                Calculate Frozen Linear Output
                '''
                for frozen_participant_index in range(N_Participants):
                    frozen_network = frozen_net_list[frozen_participant_index]
                    frozen_network = nn.DataParallel(frozen_network, device_ids=device_ids).to(device)
                    frozen_network.eval()
                    with torch.no_grad():
                        images = images.to(device)
                        frozen_linear_output = frozen_network(images)
                        linear_output_frozen_list.append(frozen_linear_output)
                        linear_output_frozen_softmax = F.softmax(frozen_linear_output, dim=1)
                        linear_output_frozen_softmax_list.append(linear_output_frozen_softmax.clone().detach())
                '''
                ...
                for progressive_participant_index in range(N_Participants):
                    ...
                    progressive_network.train()
                    with torch.no_grad():
                        images = images.to(device)
                        progressive_linear_output = progressive_network(images)
                        linear_output_progressive_list.append(progressive_linear_output)
                        linear_output_progressive_softmax = F.softmax(progressive_linear_output, dim=1)
                        linear_output_progressive_softmax_list.append(linear_output_progressive_softmax.clone().detach())
                ...
                for participant_index in range(N_Participants):
                    '''
                    Calculate the Weight with others and itself
                    '''
                    '''
                    Calculate Participant Dissimilarity ! Now
                    '''
                    weight_list = []
                    for q in range(N_Participants):
                        if participant_index == q:
                            weight_index = calculate_class_similarity(linear_ouput_softmax_list[participant_index],
                                                                  linear_output_frozen_softmax_list[q], len(images))
                        else:
                            weight_index = calculate_class_similarity(linear_ouput_softmax_list[participant_index],
                                                                  linear_ouput_softmax_list[q], len(images))
                        weight_index = 1 - weight_index
                        weight_list.append(weight_index)
                    weight_list = torch.tensor(weight_list)
                    softmax_weight_list= F.softmax(weight_list,dim=0)
                    ...
                    network = nn.DataParallel(network, device_ids=device_ids).to(device)
                    network.train()
                    optimizer = optim.Adam(network.parameters(), lr=Pariticpant_Params['learning_rate'],weight_decay=0.0001)
                    optimizer.zero_grad()
                    linear_output_target_weight_avg_list = []
                    for i in range(N_Participants):
                        if i != participant_index:
                            linear_output_target_weight_avg_list.append(linear_output_target_list[i]*softmax_weight_list[i])
                        if i ==participant_index:
                            linear_output_target_weight_avg_list.append(linear_output_progressive_list[i]*softmax_weight_list[i])

                    linear_output_target_avg = torch.sum(torch.stack(linear_output_target_weight_avg_list),dim=0)
