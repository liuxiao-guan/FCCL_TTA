...
from Idea.utils_idea_ntkd import _train_net, update_model_via_private_data_with_two_model,evaluate_network,mkdirs
...
Ablation_Name='Barlow_HSIC_FISL_NTKD'
    ...
    for epoch_index in range(CommunicationEpoch):
       ...
       # 开始全局训练
        ...
        for pub_epoch_idx in range(Public_Training_Epoch):
            for batch_idx, (images, _) in enumerate(public_train_dl):
                ...
                for participant_index in range(N_Participants):
                    ...
                    optimizer.zero_grad()
                    col_loss = loss_distill+fccl_loss
        ...
        for participant_index in range(N_Participants):
            ...
            # _train_net(index,net,inter_net,train_loader,local_lr,logger):
            network, private_loss_batch_list = _train_net(participant_index,network,progressive_network,train_dl_local,Pariticpant_Params['learning_rate'],logger=logger)
