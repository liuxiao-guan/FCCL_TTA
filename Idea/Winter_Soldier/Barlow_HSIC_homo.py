...
Ablation_Name='Barlow_HSIC_homo'
...
if __name__ =='__main__':
    ...
    for epoch_index in range(CommunicationEpoch):
        ...
        for idx in range(N_Participants):
            local_weights = net_list[idx].state_dict()
            for key in net_list[0].state_dict().keys():
                if idx == 0:
                    averaged_weights[key] = 1 / N_Participants * local_weights[key]
                else:
                    averaged_weights[key] += 1 / N_Participants * local_weights[key]
        for idx in range(N_Participants):
            net_list[idx].load_state_dict(averaged_weights)
